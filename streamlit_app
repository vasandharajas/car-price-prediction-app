...
model = joblib.load(r"/workspaces/car-price-prediction-app/xgboost_ml_model.pkl")
encoded_columns = joblib.load(r"/workspaces/car-price-prediction-app/encoded_columns.pkl")

# Load dataset from Excel to extract unique values for dropdown options
df_cars = pd.read_excel(r"/workspaces/car-price-prediction-app/Preprocessed_data.xlsx")

# Print columns to verify
print("Columns in DataFrame:", df_cars.columns)
...
def main():
    ...
    image_path = r'/workspaces/car-price-prediction-app/logo.png'
    if os.path.exists(image_path):
        st.image(image_path, width=700)
    else:
        st.error("Image file not found: 'logo.png'")
    ...
    city = st.sidebar.selectbox('city', unique_values['city'], index=0)
